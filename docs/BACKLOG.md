...
| ID | Task | Owner | Priority | Status | Est | Depends On | Acceptance |
|---|---|---|---|---|---|---|---|
| M6-01 | 预算守门器（按 tier 限 tool_calls/cost） | BE | P0 | DONE | 1d | M5 | 超预算自动降级并记录原因 |
| M6-02 | 限流与重试策略（外部数据源/LLM） | BE | P0 | DONE | 1d | M6-01 | 429/超时可控重试，不雪崩 |
| M6-03 | 降级矩阵实现（数据缺失/模型失败/图谱不可用） | BE | P0 | DONE | 1d | M6-02 | 任一依赖故障可返回保守报告 |
| M6-04 | 全链路 trace 与审计日志落库（tool/model/version） | BE + SRE | P0 | DONE | 1d | M6-01 | 每次生成可追踪来源与耗时成本 |
| M6-05 | 指标面板（成功率/延迟/失败率/成本/schema pass） | SRE | P1 | DONE | 1d | M6-04 | 仪表盘可查看近 7 天趋势 |
| M6-06 | 告警规则（失败率、延迟、成本、schema fail） | SRE | P0 | DONE | 0.5d | M6-05 | 告警触发准确，含分级策略 |
| M6-07 | runbook 补全（故障处理、回滚、恢复、升级） | SRE + BE | P1 | DONE | 0.5d | M6-06 | 值班人员可按文档完成处置 |
| M6-08 | 压测（并发 + 长跑）与容量基线 | QA + SRE | P1 | DONE | 1d | M6-03,M6-05 | 达到 SLA 并输出容量建议 |
| M6-09 | 风控门禁回归测试（边界条件与覆盖率） | QA + BE | P0 | DONE | 1d | M6-03 | risk_gate 对 LLM 有最高优先级 |
| M6-10 | 上线预演（故障演练 + 数据回放演练） | SRE + QA | P0 | DONE | 0.5d | M6-07,M6-08,M6-09 | 演练通过并记录改进项 |
