...
def test_consistency_rejects_invalid_ta_horizon_hint() -> None:
    report = {
        'evidence_refs': [
            {
                'evidence_id': 'ev_1',
        ...
        'price_bands': [],
        'data_quality': {'status': 'OK'},
        'decision': {'action': 'WATCH', 'confidence': 0.55},
        'provenance': {
            'ta_hybrid': {
                'applied': False,
                'require_evidence_refs': True,
                'disagreement': 0.12,
                'horizon_days_hint': 180,
            }
        },
    }
    errors = check_consistency(report)
    assert any('ta_hybrid.horizon_days_hint must be within [1, 120]' in err for err in errors)
